...
const features = [
  {
    icon: '🩸',
    title: '记录每次测量',
    desc: '空腹、餐前、餐后、睡前都能分别记录，避免忘记',
  },
  {
    icon: '📊',
    title: '趋势一眼看懂',
    desc: '按天/时间段查看血糖变化，辅助发现异常波动',
  },
  {
    icon: '💊',
    title: '关联用药',
    desc: '记录是否用了降糖药或胰岛素，方便医生回顾',
  },
  {
    icon: '🔒',
    title: '隐私与安全',
    desc: '所有数据仅存本地，不上传云端，随时可以导出或删除',
  },
]
...
export default function BloodSugarIntro() {
  ...
  const handleAddHabit = async () => {
    ...
    if (hasAdded) {
      await navigateTo('https://tinyhabits.top/habit/bloodSugar')
      return
    }
    ...
    try {
      ...
      const result = await callNative('habit.create', {
        ...
        icon: 'ic_habit_blood_sugar',
        bgColor: '#0EA5E9',
        description: '记录每日血糖与用药情况，守护血糖稳定',
      })
      await hideLoading()
      ...
      if (isSuccess) {
        ...
        const errorMsg = result?.message || '添加失败，请重试'
        console.error('[BloodSugarIntro] 添加失败:', errorMsg, result)
        await showToast(errorMsg)
      }
    } catch (error) {
      await hideLoading()
      console.error('[BloodSugarIntro] 添加异常:', error)
      await showToast('添加失败: ' + (error.message || '未知错误'))
    } finally {
      setIsAdding(false)
    }
    ...
    return (
      <div className="min-h-screen bg-gradient-to-b from-sky-50 to-white flex items-center justify-center p-4">
        <div className="text-center">
          ...
          <a
            href="https://apps.apple.com/app/id1455083310"
            className="inline-block px-6 py-2 bg-sky-500 text-white rounded-full text-sm"
          >
            下载小习惯 App
          </a>
        </div>
  ...
  return (
    <div className="min-h-screen bg-gradient-to-b from-sky-50 via-white to-sky-50">
      {/* 头部装饰 */}
      <div className="relative pt-8 pb-12 px-6">
        <div className="absolute top-0 left-0 w-full h-48 bg-gradient-to-b from-sky-100/60 to-transparent" />
        ...
        <div className="relative text-center">
          <h1 className="text-2xl font-bold text-gray-800 mb-2">血糖记录</h1>
          <p className="text-gray-500 text-sm">记录·分析·守护血糖稳定</p>
        </div>
      </div>
      ...
      <div className="px-6 pb-8">
        <div className="bg-white rounded-2xl p-5 shadow-sm">
          ...
          <ul className="text-sm text-gray-600 space-y-2">
            ...
            <li className="flex items-start gap-2">
              <span className="text-sky-500 mt-0.5">•</span>
              <span>建议按医生要求，在固定时间点记录血糖值</span>
            </li>
            <li className="flex items-start gap-2">
              <span className="text-sky-500 mt-0.5">•</span>
              <span>可以一并记录用药、胰岛素剂量和备注情况</span>
            </li>
            <li className="flex items-start gap-2">
              <span className="text-sky-500 mt-0.5">•</span>
      ...
      <div className="fixed bottom-0 left-0 right-0 p-4 bg-gradient-to-t from-white via-white to-transparent">
        <div className="max-w-md mx-auto">
          {hasAdded ? (
            <button
              onClick={handleEnter}
              className="w-full py-4 bg-gradient-to-r from-sky-500 to-emerald-400 text-white rounded-xl font-medium shadow-lg active:scale-98 transition-transform"
            ...
            <button
              onClick={handleAddHabit}
              disabled={isAdding}
              className="w-full py-4 bg-gradient-to-r from-sky-500 to-emerald-400 text-white rounded-xl font-medium shadow-lg active:scale-98 transition-transform disabled:opacity-70"
              style={{ boxShadow: '0 4px 20px rgba(14, 165, 233, 0.45)' }}
            >
              {isAdding ? '添加中...' : '添加到首页'}
            </button>
